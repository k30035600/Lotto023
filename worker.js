 ...
 * This script runs in a background thread and performs heavy calculations
 * to avoid blocking the main UI thread.
 */

// All functions from the original statistics.js are placed here.
// These are pure functions that take data and return a result without
// interacting with the DOM or global state.
 ...
 * Receives data from the main thread, performs calculations, and sends the result back.
 */
self.onmessage = function(e) {
    const { lottoData, maxNumber } = e.data;

    try {
        const statistics = initializeStatistics(lottoData, maxNumber);

        // Convert Maps to serializable format (Array of [key, value] pairs)
        const serializableStats = {
            ...statistics,
            winStatsMap: Array.from(statistics.winStatsMap.entries()),
            appearanceStatsMap: Array.from(statistics.appearanceStatsMap.entries()),
            winPercentageMap: Array.from(statistics.winPercentageMap.entries()),
            appearancePercentageMap: Array.from(statistics.appearancePercentageMap.entries()),
        };

        // Post the result back to the main thread
        self.postMessage({
            type: 'SUCCESS',
            payload: serializableStats
        });
    } catch (error) {
        self.postMessage({
            type: 'ERROR',
